...
Fork này dùng `master` làm branch chính (không có `dev`/`main` như upstream). Branch off `master` với prefix `feat/`, `fix/`, `refactor/`, `chore/`. Commit format: `type(scope): description`. Release flow: bump `version` trong `ida-plugin.json` → tag `v*`.
