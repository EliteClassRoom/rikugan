...
5. Đợi CI workflow (lưu ý: `.github/workflows/ci.yml` chỉ trigger trên `[main, dev]` upstream — push lên fork master **không** kích hoạt, cần chạy `./ci-local.sh` trước)

> **Lưu ý remote:** `origin` → fork `EliteClassRoom/rikugan` (master), `tuna-main` → upstream `tuna1999/Rikugan` (main). Đừng push nhầm remote. Lưu ý: file `.github/workflows/ci.yml` vẫn trigger trên `[main, dev]` (từ upstream) — nên push lên `master` của fork **không** kích hoạt CI; chạy `./ci-local.sh` trước khi push.
...
- **Skills**: Markdown + YAML frontmatter trong `rikugan/skills/builtins/<slug>/SKILL.md`. User tự thêm vào `~/.idapro/rikugan/skills/`. 10 skill built-in: `malware-analysis`, `linux-malware`, `deobfuscation`, `ctf`, `modify`, `smart-patch-ida`, `vuln-audit`, `ida-scripting`, `driver-analysis`, `generic-re`.
...
- **IDAPython docs-review gate** (origin `4295fdc`): complex `execute_python` scripts (multi-module, mutating, Hex-Rays / types / frames / UI / domain APIs) tự động đi qua docs-reviewer subagent (`rikugan/agent/agents/ida_docs_reviewer.py`) TRƯỚC khi user approval. Reviewer block scripts dựa trên hallucinated APIs. Configurable qua `require_ida_docs_for_complex_scripts` (default `True`) trong Settings.
- Blocklist patterns (`subprocess`, `os.system`, `os.popen`, `os.exec*`, `os.spawn*`, `Popen`, `__import__("subprocess")`) → thêm vào `BLOCKED_SCRIPT_PATTERNS` trong `script_guard.py`
