...
def write_and_review_note(
    ...
    review_prompt = (
        ...
        f"- `decompile_function` or `read_disassembly` to check that "
        f"functions at cited addresses actually exist and do what the note claims\n"
        f"- `search_functions` to verify function names are real\n"
        f"- `list_imports` / `list_strings` to verify import/string claims\n"
        f"- `xrefs_to` to verify cross-reference claims\n\n"
    ...
    try:
        ...
        if passed:
            ...
            rewrite_prompt = (
                ...
                f"(decompile_function, read_disassembly, search_functions, "
